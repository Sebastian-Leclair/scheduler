...
/** This class acts as a controller for the UpdateAppointment view.*/
public class UpdateAppointment implements Initializable {
    ...
    /**
     * An event to set the end date to the start date when the start date is selected.
     * @param event
     */
    @FXML
    ...
    /**
     * An event to update an appointment in the database.
     * @param event
     * @throws IOException
     * @throws SQLException
     */
    ...
    void onActionUpdateAppointment(ActionEvent event) throws IOException, SQLException {
        ...
        if (startDisplay.atZone(ZoneId.of("America/New_York")).toLocalTime().isBefore(LocalTime.parse("08:00")) || endDisplay.atZone(ZoneId.of("America/New_York")).toLocalTime().isBefore(LocalTime.parse("08:00")) || startDisplay.atZone(ZoneId.of("America/New_York")).toLocalTime().isAfter(LocalTime.parse("22:00")) || endDisplay.atZone(ZoneId.of("America/New_York")).toLocalTime().isAfter(LocalTime.parse("22:00")) || start.toLocalTime() == end.toLocalTime() || startDayOfWeek == "SUNDAY" || startDayOfWeek == "SATURDAY") {
            ...
            AppointmentsQuery.insert(Integer.parseInt(appointmentIdTxt.getText()), titleTxt.getText(), descriptionTxt.getText(), locationTxt.getText(), typeCombo.getValue(), start, end, customerIdCombo.getValue().getCustomerId(), userIdCombo.getValue().getUserId(), contactCombo.getValue().getContactId());
    ...
    /**
     * A method to send appointment information to the Main view.
     * @param appointment
     * @throws SQLException
     */
    public void sendAppointment(Appointment appointment) throws SQLException {
        ...
        contactCombo.setValue(ContactsQuery.getAppointmentContact(appointment.getContactName()));
        typeCombo.setValue(appointment.getType());
        startDatePicker.setValue(appointment.getStart().toLocalDate());
        endDatePicker.setValue(appointment.getStart().toLocalDate());
        startTimeCombo.setValue(String.valueOf(appointment.getStart().toLocalTime()));
        endTimeCombo.setValue(String.valueOf(appointment.getEnd().toLocalTime()));
        customerIdCombo.setValue(CustomersQuery.getAppointmentCustomer(appointment.getCustomerId()));
        userIdCombo.setValue(UsersQuery.getAppointmentUser(appointment.getUserId()));
    }
